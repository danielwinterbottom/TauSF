...
if [ "${working_pointVsEle}" == True ]; then
  python scripts/harvestDatacards.py --dm-bins -o "outputs/${output_dir}" --wp "${working_pointVsJets}" --tightVsEle

else
  python scripts/harvestDatacards.py --dm-bins -o "outputs/${output_dir}" --wp "${working_pointVsJets}"
fi
